...
const DashboardV2: React.FC<DashboardV2Props> = ({ restaurantData, onNavigate }) => {
    ...
    return (
        <div className="space-y-8">
            {/* Section intro */}
            <div>
                <h3 className="text-xl sm:text-2xl font-bold text-ink tracking-tight flex items-center gap-2">
                ...
                <div className="bg-surface rounded-3xl p-6 border border-line">
                    ...
                    <dl className="mt-5 space-y-4">
                        <div className="flex items-center justify-between gap-4">
                            <dt className="text-base text-muted">Orders</dt>
                            <dd className="text-lg font-semibold text-ink tabular-nums">{stats.todays}</dd>
                        </div>
                        <div className="h-px bg-line" />
                        <div className="flex items-center justify-between gap-4">
                            <dt className="text-base text-muted">Revenue</dt>
                            <dd className="text-lg font-semibold text-ink tabular-nums">{inr(stats.revenueToday)}</dd>
                        </div>
                        <div className="h-px bg-line" />
                        <div className="flex items-center justify-between gap-4">
                            <dt className="text-base text-muted">Avg. order value</dt>
                            <dd className="text-lg font-semibold text-ink tabular-nums">{inr(stats.avgOrder)}</dd>
                        </div>
                    </dl>
                </div>
            ...
            <div className="bg-surface rounded-3xl p-6 border border-line">
                ...
                {attention.length === 0 ? (
                    <p className="flex items-center gap-2.5 text-base text-muted mt-4">
                        <span className="w-6 h-6 rounded-full bg-success/15 text-success flex items-center justify-center text-sm" aria-hidden="true">✓</span>
                            ...
                            <li key={i}>
                                <div className="flex items-start justify-between gap-4 py-3 px-3 -mx-3 rounded-xl hover:bg-primary-soft/40 transition-colors">
                                    ...
                                    <button
                                        ...
                                        className="shrink-0 px-3.5 py-1.5 rounded-lg text-sm font-semibold text-primary-strong bg-primary-soft hover:bg-primary hover:text-white transition-colors"
                                    >
                                        Review
                                    </button>
                                </div>
