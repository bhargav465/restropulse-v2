 ...
 *
 * Loads .env before any other module runs so packages that capture
 * process.env values as module-level constants see correct values at
 * initialisation time. Initializes Azure Monitor OpenTelemetry
 * auto-instrumentation (MongoDB, HTTP) and the structured pino logger.
 *
 * Mirrors apps/publisher/src/instrument.ts.
 */
